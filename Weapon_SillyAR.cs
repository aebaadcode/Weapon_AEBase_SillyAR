// BNE_SillyAR
datablock DebrisData(AEBNE_SillyARMagDebris)
{
	shapeFile = "./SillyAR/SillyARMag.dts";
	lifetime = 2.0;
	minSpinSpeed = -150.0;
	maxSpinSpeed = -100.0;
	elasticity = 0.5;
...
// item //
//////////
datablock ItemData(BNE_SillyARItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./SillyAR/SillyAR.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_SillyARImage)
{
   // Basic Item properties
   shapeFile = "./SillyAR/SillyAR.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = BNE_SillyARSafetyImage;
    scopingImage = BNE_SillyARIronsightImage;
	doColorShift = true;
	colorShiftColor = BNE_SillyARItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function BNE_SillyARImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
...
function BNE_SillyARImage::onReloadEnd(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function BNE_SillyARImage::onReload2End(%this,%obj,%slot)
{
    %obj.schedule(150, playAudio, 1, BNE_AR15BoltRackSound);
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function BNE_SillyARImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_SillyARImage::onReloadMagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "plant");
}

function BNE_SillyARImage::onReload2MagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "plant");
   %obj.schedule(400, "aeplayThread", "3", "shiftleft");
}

function BNE_SillyARImage::onReload2Bolt(%this,%obj,%slot)
{
   %obj.aeplayThread(2, plant);
   %obj.schedule(200, "aeplayThread", "3", "shiftleft");
}

function BNE_SillyARImage::onReloadMagOut(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
}

function BNE_SillyARImage::onReload2MagOut(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
}
...
function BNE_SillyARImage::onReloadStart(%this,%obj,%slot)
{
   %obj.aeplayThread(2, plant);
...
function BNE_SillyARImage::onReady(%this,%obj,%slot)
{
...
function BNE_SillyARImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_SillyARImage::onUnMount(%this,%obj,%slot)
{
...
function BNE_SillyARImage::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
	...
	%a.mountImage(BNE_SillyARMagImage,0);
...
datablock ShapeBaseImageData(BNE_SillyARMagImage)
{
...
function BNE_SillyARMagImage::onDone(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(BNE_SillyARSafetyImage)
{
   shapeFile = "./SillyAR/SillyAR.dts";
   emap = true;
   mountPoint = 0;
   ...
   armReady = false;
   hideHands = false;
   safetyImage = BNE_SillyARImage;
   doColorShift = true;
   colorShiftColor = BNE_SillyARItem.colorShiftColor;
...
function BNE_SillyARSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_SillyARSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
datablock ShapeBaseImageData(BNE_SillyARIronsightImage : BNE_SillyARImage)
{
	recoilHeight = 0.0625;

	scopingImage = BNE_SillyARImage;
	sourceImage = BNE_SillyARImage;
...
function BNE_SillyARIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_SillyARIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_SillyARIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_AR15Fire @ getRandom(1, 4) @ Sound);
...
function BNE_SillyARIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_SillyARIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_SillyARIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
